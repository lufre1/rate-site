...
- **Description cleanup**: Removes a leading/trailing or comma-adjacent "oder"/"or"; normalizes whitespace. An **infix** "oder" is deliberately left in place — "Basmatireis oder Trüffel Pommes" is a *choice*, and rewriting it to a comma list would tell the reader both sides are served
- **Side extraction** (`_extract_side_parts`): a main dish's description is split on commas **and** on a bare "oder" (whitespace on both sides, so "moderne Beilage" survives), then each part is filtered — a lone conjunction, a part starting with "mit " (it continues the dish above it), sauces/dressings/ketchup/remoulade/mayo/senf, kompott/topping/zusätzlich, and anything under 3 characters. An **infix** "mit" is kept: it belongs to the side's own name ("Kartoffelsalat mit Ei und Gurke"). `name_en` repeats the German text — pairing DE/EN parts by index mislabels sides (19% of live description pairs split into different part counts), see #21
- **Stale sides** (`SCRAPER_PRUNE_SIDES`, default off): sides are extracted, not listed on the page, so `_reconcile` used to skip every `type='side'` row and a mis-split side stayed rateable forever. With the env var set, a side the run did not extract is reconciled like any other row — unless its name is in `german_names` (a page-listed "Beilage"/"Salat" row is typed `side` too) or extraction never ran for that mensa/date (an empty page must not wipe a day's sides)
...
- **Only a token digest is stored** — `auth_tokens.token` holds `auth._digest(token)` (SHA-256), never the token itself, so a `pg_dump` is not a set of usable credentials. The plaintext exists only in the login response and the client's `localStorage`; nothing server-side can recover it. Anything querying that table by token value must digest first — `logout` does
- **Tokens carry a 90-day sliding expiry** — `auth.TOKEN_TTL`. `_lookup` rejects a lapsed row *and* a row whose `expires_at` is `NULL` (fail closed), and pushes the expiry out as the session is used, guarded to one write per token per day so `_lookup` does not write on every read. A daily `purge_expired_tokens` job on the scheduler reclaims lapsed rows; it is housekeeping, not enforcement
- **The `expires_at` migration purges `auth_tokens` once** (`init_db`) — gated on the column being absent so it runs exactly once. Deliberate: a digest is deterministic, so hashing the old plaintext rows in place would have left every token already copied into the nightly `pg_dump`s valid. Deleting them is what retires them
